...
**Inspected 2026-08-22.** Target is `Cypha/native/.../adaptive_predictor_mixer.{hpp,cpp}` — 7 default experts on the predictive-codec path, not `eval_bpc`. No dump exists. Do **not** point `hp/tools/dump_experts` at Cypha; add Cypha accessors and a sibling tool that emits the same int16 layout `mp_rank.py` already consumes. `gria_gated_blend_logit` is a different, upstream 2-way blend (`use_gria_gated_mixture`, default off).

See `RECORD.md` harvest follow-up.
...
**`--mem` must derive match/buf on decode.** Scaling those tables in the CLI
only desynchronised the coder. `Config::normalize()` is now the single rule:
`match_bits = table_bits`, `buf_bits = table_bits + 4` (22 → 26, historical).

**A p-cache cannot be bit-identical.** B.1 is a `hash2` last-value memo plus
a pattern-class taxonomy. On/off archives matched on proxy64k.

**B.3 remains NO-GO.** Dictionary lesson stands; enwik8 is on disk but the
full 100 MB run is the only scale that can flip 4.1.

**Synthetic `gen_proxy.py` over-compresses** (0.59–0.70 bpc). Do not treat it
as a replacement for the missing 3.1 MB proxy or for enwik8.

**PPMD o25 not built** (`HP_PPMD=0`). PY backoff + new CTW already read that
axis; A.3 would likely reject it as saturated. Recorded as a predicted reject
with no rank delta yet (no dump).

Ports behind flags, all default ON except PPMD and nibble buckets: wiki
states, word-match ×3, word streams, bracket, per-mixer lr, pattern cache,
meta-pattern skip-k + `pat_`, NCL λ=4/256, Hedge-L1, CTW, English prior.
Mixer inputs 65. Gates 8. `kVersion = 3`.
